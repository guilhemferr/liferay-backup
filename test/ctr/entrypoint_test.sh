...
MYSQLDUMP=/portal_backup/valid.tgz
...
function populate_vol() {
  ...
  for target in $t; do
    ...
    t2=${t/SEQ/${seqno}}
    mkdir -p /portal_backup/${seqno}/data
    chmod -R 0777 /portal_backup/${seqno}
    echo "target: ${t2}" >>/portal_backup/${seqno}/list

    # are we working with nopath?
    if [[ "$t2" =~ nopath ]]; then
      rm -f /portal_backup/nopath
      ln -s ${seqno}/data /portal_backup/nopath
    fi
...
function populate_pre_post() {
  ...
  mkdir -p /portal_backup/${sequence}/{pre-backup,post-backup,pre-restore,post-restore}
  echo touch /portal_backup/${sequence}/post-backup/post-backup.txt >/portal_backup/${sequence}/post-backup/test.sh
  echo touch /portal_backup/${sequence}/post-restore/post-restore.txt >/portal_backup/${sequence}/post-restore/test.sh
  echo touch /portal_backup/${sequence}/pre-backup/pre-backup.txt >/portal_backup/${sequence}/pre-backup/test.sh
  echo touch /portal_backup/${sequence}/pre-restore/pre-restore.txt >/portal_backup/${sequence}/pre-restore/test.sh
  chmod -R 0777 /portal_backup/${sequence}
  chmod 755 /portal_backup/${sequence}/*/test.sh
}
...
function checktest() {
  ...
  POST_BACKUP_OUT_FILE="/portal_backup/${sequence}/post-backup/post-backup.txt"
  PRE_BACKUP_OUT_FILE="/portal_backup/${sequence}/pre-backup/pre-backup.txt"
  POST_RESTORE_OUT_FILE="/portal_backup/${sequence}/post-restore/post-restore.txt"
  PRE_RESTORE_OUT_FILE="/portal_backup/${sequence}/pre-restore/pre-restore.txt"
  ...
  for target in $t; do
    ...
    bdir=/portal_backup/${seqno}/data # change our target
    if [[ "$DEBUG" != "0" ]]; then
      ls -la $bdir >&2
    fi
    ...
    TMP1=/portal_backup/check1
    TMP2=/portal_backup/check2

    BACKUP_FILE=$(ls -d1 $bdir/portal_backup_*.tgz 2>/dev/null)
    ...
    if [[ ! -d "$bdir" ]]; then
      ...
      gunzip <$MYSQLDUMP | tar -xOf - | sed -e 's/^\/\*![0-9]\{5\}.*\/;$//g' | sed 's/^.*SET character_set_client.*$//g' | cat >$TMP2
...
help)
  echo
  echo "Consider using: [ prepare_pre_post | populate | check | check_source_target | save_dump | cron | help ]"
  echo
  ;;
*)
  echo
  echo "[ERROR] failed to process command: $cmd"
  echo
  exit 1
  ;;
esac
