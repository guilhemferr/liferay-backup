...
BACKUP_DIRECTORY_BASE=/tmp/portal_backup.$$
...
for ((i=0; i< ${#targets[@]}; i++)); do
        t=${targets[$i]}
        docker run --label mysqltest --name mysqlbackup-data-populate --rm -v ${BACKUP_VOL}:/portal_backup -e DEBUG=${DEBUG} ${BACKUP_TESTER_IMAGE} populate "$t" $seq
...
for ((i=0; i< ${#targets[@]}; i++)); do
	t=${targets[$i]}
        results=$(docker run --label mysqltest --name mysqlbackup-data-check --rm -v ${BACKUP_VOL}:/portal_backup -e DEBUG=${DEBUG} ${BACKUP_TESTER_IMAGE} check_source_target "$t" $seq $(get_default_source))
